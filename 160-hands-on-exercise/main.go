...
func (md MockDatastore) GetUser(id int) (User, error) {
	v, ok := md.Users[id]
	if !ok {
		return User{}, fmt.Errorf("User not present with Id=%v", id)
	}
	return v, nil
}
...
func (md MockDatastore) SaveUser(u User) error {
	_, ok := md.Users[u.Id]
	if ok {
		return fmt.Errorf("User already present with Id=%v", u.Id)
	}
	md.Users[u.Id] = u
	return nil
}
...
func (s Service) SaveUserToDS(u User) error {
...
func main() {
	...
	fmt.Println(service.SaveUserToDS(user))
...
func (s Service) SaveUsers(u User) error {
	return s.ds.SaveUser(u)
}
...
func main() {
	...
	err := srvc.SaveUsers(u1)
	if err != nil {
		log.Fatalf("error %s", err)
	}

	u1Returned, err := srvc.GetUsers(u1.ID)
	if err != nil {
		log.Fatalf("error %s", err)
	}
